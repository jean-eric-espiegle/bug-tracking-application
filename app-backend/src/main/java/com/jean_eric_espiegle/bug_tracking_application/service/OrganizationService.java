...
public class OrganizationService {
    ...
    public OrganizationService(OrganizationRepository organizationRepository, MembershipRepository membershipRepository, UserRepository userRepository) {
    ...
    private Membership findMembership(User user, Organization organization) {
        return membershipRepository.findByUserAndOrganization(user, organization)
                .orElseThrow(() -> new IllegalStateException("User " + user.getUsername() + " is not a member of organization " + organization.getName()));
    }
    ...
    public void addMember(Long organizationId, AddMemberRequest request, User currentUser) {
        ...
        // 1. Check if current user has permission to add members (e.g., is OWNER or ADMIN)
    ...
    private void checkPlanLimits(Organization organization, Role newRole) {
        ...
        if (totalUsers >= plan.getMaxUsers()) {
            throw new IllegalStateException("Cannot add new member. Organization has reached its maximum user limit for the " + plan.getType() + " plan.");
        }
        ...
        if (newRole == Role.ADMIN) {
            ...
            if (adminCount >= plan.getMaxAdmins()) {
                throw new IllegalStateException("Cannot add new admin. Organization has reached its maximum admin limit for the " + plan.getType() + " plan.");
            }
        }
        ...
        if (newRole == Role.SUPPORT) {
            ...
            if (supportCount >= plan.getMaxSupport()) {
                throw new IllegalStateException("Cannot add new support member. Organization has reached its maximum support limit for the " + plan.getType() + " plan.");
            }
        }
    }
}
